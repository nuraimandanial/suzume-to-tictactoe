...
export default function TTT() {
  ...
  useEffect(() => {
    ...
    if (email) {
      ...
      (async () => {
        const board = await obj.ExtractLoadedBoard(email, "tictactoe");
        if (board) {
          setLoadGameCriteria((prev) => ({
            ...prev,
            board: board.board,
          ...
          {loadGameCriteria.board.length > 0 ? (
            loadGameCriteria.board.map((insideBoard, index) => {
              return (
                <section
                  key={nanoid()}
                  className="snap-center w-[40rem] z-[3] font-extrabold grid grid-cols-[6rem_6rem_6rem] justify-center place-items-center"
                >
